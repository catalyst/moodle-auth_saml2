...
class auth_plugin_saml2 extends auth_plugin_base {
    ...
    public function validate_form($form, &$err) {
        ...
        $form->idpmetadata = trim($form->idpmetadata);
        ...
        $idps = $this->parse_idpmetadata($idpmetadata);
        ...
        if (count($idps) > 1) {
            set_config('duallogin', '1', 'auth/saml2');
        }
     ...
     */
    private function parse_idpmetadata($data) {
        $idps = [];
